...
# def download_write_video(name, url, timeout):
def download_write_video(name, url, num):
    try:
        name = num + ' - ' + name
        with open(name, 'wb') as f:
            # f.write(requests.get(url, headers=headers, timeout=timeout).content)
        ...
        if works_count and li_list_count:
            ...
            if len(already_video_name_list) >= works_count:
                ...
                for i in range(len(video_urls_list) - 1, -1, -1):
                    ...
                    download_write_video(name=video_name_list[i], url=video_urls_list[i], num=num)
                    print(
